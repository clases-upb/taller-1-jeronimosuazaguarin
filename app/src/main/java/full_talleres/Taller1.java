...
//            return String.format("%.1f m/s - %.2e m/h", velocidad_ms, velocidad_mh);
    ...
    */
     ...
     public static float Convertir_cm_lt(double cc){
        ...
        final short FACTOR_CONVERSION_CM_LT = 1000;
        try {
            litros = (float) (cc / FACTOR_CONVERSION_CM_LT);
            return litros;
        }
        catch (Exception e){
            return litros;
...
public static int Calcular_segs(short dias, short horas, short minutos, short segundos){
    ...
    final short DIAS_A_HORAS = 24;
    final short HORAS_A_MINUTOS = 60;
    final short MINUTOS_A_SEGUNDOS = 60;

    try {
        int dias_segundos = dias * DIAS_A_HORAS * HORAS_A_MINUTOS * MINUTOS_A_SEGUNDOS;
        int horas_segundos = horas * HORAS_A_MINUTOS * MINUTOS_A_SEGUNDOS;
        int minutos_segundos = minutos * MINUTOS_A_SEGUNDOS;
...
//            return String.format("%.1f kilos - %.1f toneladas", peso_neto_kg, peso_neto_ton);
   ...
   Si hay algún error, devuelva -1.
   */
    public static float Cal_horas_POR_viaje(String destino, short distancia, short velocidad){
    float horas = -1;
      try {
   ...
   public static float Calcular_combustible(int ruta1, int ruta2, int ruta3, int ruta4){
    float total_combustible = 0;
     ...
     final float PRECIO_DESPEGUE = 1.2F;
     final float PRECIO_ATERRIZAJE = 0.4F;
    ...
    try {
        for (int ruta: rutas){
            total_combustible += PRECIO_DESPEGUE;
            total_combustible += ruta * COMBUSTIBLE_POR_KM;
            total_combustible += PRECIO_ATERRIZAJE;
        }
   ...
   */
    ...
    double peso_luna = 0;
      ...
      final double CONVERSION_KG_NEW = 9.81;
      final double CONVERSION_PESO_LUNA = 0.165;
    try {
        peso_luna = (peso_tierra_kg * CONVERSION_KG_NEW) * CONVERSION_PESO_LUNA;
